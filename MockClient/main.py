...
from cryptography.hazmat.primitives.asymmetric import padding as asymmetric_padding, padding
...
def formatted_asymmetric_message(key: RSAPublicKey, data: bytes, transmission_id: int = 0, flags: int = 0b00000000,
                                 encryption_method: int = 1, request_num: int = 0):
...
if __name__ == '__main__':
    ...
    server_address = ('10.0.0.141', 7000)
    ...
    try:
        ...
        print(public_pem)
        public_pem = public_pem[: public_pem.index(0x04)]
        server_public_k = serialization.load_pem_public_key(public_pem, backend=default_backend())
        our_private_k = serialization.load_pem_private_key(PEM_RSAPrivateKey.encode('utf-8'), password=None, backend=default_backend())
        # Send data to the server
        message_header_encrypted, message_body_encrypted, encrypted_body_chunk_list, message_header, data, body_chunk_list = formatted_asymmetric_message(server_public_k,PEM_RSAPublicKey.encode())
        client_socket.sendall(message_header_encrypted+message_body_encrypted)

        our_private_k.decrypt(client_socket.recv(our_private_k.key_size // 8), padding.PKCS1v15())
        print(our_private_k.decrypt(client_socket.recv(our_private_k.key_size), padding.PKCS1v15()))
